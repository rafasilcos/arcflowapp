...
app.get('/api/briefings/:id', authenticateToken, async (req, res) => {
  try {
    ...
    // 🚀 QUERY MELHORADA - INCLUINDO VERIFICAÇÃO DE ORÇAMENTO
    const briefingQuery = `
      SELECT 
        b.id,
        b.nome_projeto as "nomeProjeto",
    ...
    // 🐛 DEBUG: Log das propriedades de orçamento
    console.log('🐛 [DEBUG] Propriedades de orçamento:', {
      temOrcamento: row.temOrcamento,
...
app.post('/api/orcamentos-inteligentes/gerar/:briefingId', authenticateToken, async (req, res) => {
  try {
    ...
    // 2. 🧠 ANÁLISE DINÂMICA AVANÇADA V3.0
    console.log('🧠 [ORCAMENTO-IA-V3] Iniciando análise dinâmica avançada do briefing...');
    ...
    // Calcular orçamento baseado nos dados extraídos
    const orcamentoCalculado = await calculator.calcularOrcamentoAdaptativo(dadosEstruturados);
    
    console.log('✅ [ORCAMENTO-IA-V3] Análise dinâmica e cálculo adaptativo concluídos:', {
      ...
      confiancaOriginal: dadosEstruturados.confianca + '%',
      confiancaFinal: orcamentoCalculado.confiancaFinal + '%',
      fallbacksAplicados: orcamentoCalculado.fallbacksAplicados?.length || 0
    });
    
    // Usar dados calculados
    const tipologia = dadosEstruturados.tipologia.toLowerCase();
    const areaEstimada = dadosEstruturados.areaConstruida;
    const complexidade = dadosEstruturados.complexidade;
    const valorTotal = orcamentoCalculado.valorTotal;
    const valorPorM2 = orcamentoCalculado.valorPorM2;
    const prazoTotal = dadosEstruturados.prazoEstimado;

    // 3. Gerar código único
    const codigo = `ORC-IA-V2-${Date.now().toString(36).toUpperCase()}`;

    // 4. Usar dados calculados pelo sistema inteligente
    const disciplinas = orcamentoCalculado.disciplinas.map(d => d.codigo);
    const composicaoFinanceira = orcamentoCalculado.composicaoFinanceira;
    const cronograma = orcamentoCalculado.cronograma;
    ...
    const proposta = {
      ...
      observacoes: `Orçamento inteligente V2 gerado automaticamente - Tipologia: ${dadosEstruturados.tipologia}, Padrão: ${dadosEstruturados.padrao}, Confiança: ${dadosEstruturados.confianca}%`,
      garantias: [
      ...
      analiseRiscos: {
        nivel: complexidade === 'alta' ? 'alto' : complexidade === 'media' ? 'medio' : 'baixo',
        fatores: ['Prazo', 'Escopo', 'Aprovações', 'Complexidade técnica'],
        mitigacao: 'Cronograma NBR 13532 com margens de segurança'
      },
      benchmarking: {
        valorMedio: valorPorM2,
        fonteReferencia: 'Tabela AEC Brasil 2025 + Análise IA',
        posicionamento: 'Baseado em dados reais do briefing',
        confianca: dadosEstruturados.confianca
      }
    };

    // 5. Salvar orçamento com dados dinâmicos
    ...
    `, [
      ...
      dadosEstruturados.areaTerreno || null,
      orcamentoCalculado.valorTotal,
      orcamentoCalculado.valorPorM2,
      dadosEstruturados.tipologia.toLowerCase(),
      ...
      briefingId,
      dadosEstruturados.clienteId,
      escritorioId,
      dadosEstruturados.responsavelId || userId,
    ...
    console.log('✅ [ORCAMENTO-IA-V2] Orçamento inteligente V2 gerado com sucesso:', {
      orcamentoId,
      briefingId,
      valorTotal: orcamentoCalculado.valorTotal,
      areaConstruida: dadosEstruturados.areaConstruida,
    ...
    res.json({
      success: true,
      message: 'Orçamento inteligente V2 gerado com sucesso!',
      version: '2.0.0',
      data: {
        orcamentoId,
        codigo,
        ...
        // DADOS EXTRAÍDOS PELA IA V2 (CORRIGIDOS)
        dadosExtaidosIA: {
          tipologia: dadosEstruturados.tipologia,
          padrao: dadosEstruturados.padrao,
          complexidade: dadosEstruturados.complexidade,
          ...
          localizacao: dadosEstruturados.localizacao,
          confianca: dadosEstruturados.confianca,
          metodologia: 'ANALISE_INTELIGENTE_V2'
        },
        ...
        disciplinasDetalhadas: orcamentoCalculado.disciplinas,
        ...
        auditoria: {
          ...
          fonteDados: 'briefing_estruturado_ia',
          confiancaAnalise: dadosEstruturados.confianca,
          totalRespostasAnalisadas: Object.keys(analyzer.extrairRespostas(briefing)).length
        }
      }
    });
...
// 🆕 ROTA SIMPLES PARA BUSCAR ORÇAMENTO POR ID
app.get('/api/orcamentos/:id', authenticateToken, async (req, res) => {
  try {
    const { id } = req.params;
    const escritorioId = req.user.escritorioId;
    
    console.log(`🔍 [ORCAMENTOS] Buscando orçamento ID: ${id} para escritório: ${escritorioId}`);
    ...
    const result = await client.query(`
      ...
      WHERE o.id = $1 AND o.escritorio_id = $2 AND o.deleted_at IS NULL
    `, [id, escritorioId]);
    ...
    // Parse JSON fields com tratamento robusto
    try {
      // Disciplinas - pode ser string simples ou JSON
      if (typeof orcamento.disciplinas === 'string') {
        if (orcamento.disciplinas.includes(',')) {
          // String simples separada por vírgula
          orcamento.disciplinas = orcamento.disciplinas.split(',').map(d => d.trim());
        } else {
          try {
            orcamento.disciplinas = JSON.parse(orcamento.disciplinas);
          } catch {
            orcamento.disciplinas = [orcamento.disciplinas];
          }
        }
      } else if (Array.isArray(orcamento.disciplinas)) {
      ...
      if (typeof orcamento.composicao_financeira === 'string') {
        try {
          orcamento.composicao_financeira = JSON.parse(orcamento.composicao_financeira);
        } catch {
          orcamento.composicao_financeira = null;
        }
      }
      
      // Cronograma
      if (typeof orcamento.cronograma === 'string') {
        try {
          orcamento.cronograma = JSON.parse(orcamento.cronograma);
        } catch {
          orcamento.cronograma = null;
        }
      }
      
      // Proposta
      if (typeof orcamento.proposta === 'string') {
        try {
          orcamento.proposta = JSON.parse(orcamento.proposta);
        } catch {
          orcamento.proposta = null;
        }
      }
      
    } catch (parseError) {
      console.warn('⚠️ [ORCAMENTOS] Erro ao parsear dados:', parseError.message);
    }

    console.log(`✅ [ORCAMENTOS] Orçamento encontrado: ${orcamento.codigo}`);
    ...
    console.error('❌ [ORCAMENTOS] Erro ao buscar orçamento:', error);
    res.status(500).json({
      success: false,
...
console.log('✅ Rota /api/orcamentos/:id adicionada com sucesso');
