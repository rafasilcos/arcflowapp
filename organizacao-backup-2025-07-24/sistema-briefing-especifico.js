...
class SistemaBriefingEspecifico {
  ...
  async avancarEtapa() {
    ...
    try {
      // Procurar botão "Próxima Etapa" usando evaluate
      const botaoProximo = await this.page.evaluate(() => {
        const botoes = Array.from(document.querySelectorAll('button'));
        return botoes.find(btn => btn.textContent?.trim() === 'Próxima Etapa');
      });
      
      if (botaoProximo) {
        await this.page.evaluate((btn) => btn.click(), botaoProximo);
  ...
  async salvarBriefing() {
    ...
    try {
      ...
      if (botaoSalvar) {
        await this.page.evaluate((btn) => btn.click(), botaoSalvar);
